...
//query para mostrar todos los grupos de un curso de un centro de un profe y que te muestre si están asignados actualmente o no 
//la diferencia entre UNION y UNION ALL es que UNION elimina duplicados
SELECT CURSO.ID_CURSO AS groupId, CURSO.NIVEL_CURSO AS groupLevel, CURSO.LETRA_CURSO AS groupLetter, 0 as groupChecked FROM CURSO
INNER JOIN CENTR ON CURSO.ID_CENTR = CENTR.ID_CENTR
INNER JOIN PROFE ON CENTR.ID_CENTR = PROFE.ID_CENTR
WHERE PROFE.ID_PROFE=1 AND CURSO.NIVEL_CURSO=1 AND CURSO.ID_CURSO NOT IN 
(SELECT CURSO.ID_CURSO FROM CURSO
			INNER JOIN REL_CURSO_ASIGN ON CURSO.ID_CURSO=REL_CURSO_ASIGN.ID_CURSO 
			INNER JOIN ASIGN ON REL_CURSO_ASIGN.ID_ASIGN=ASIGN.ID_ASIGN
            INNER JOIN PROFE ON ASIGN.ID_PROFE=PROFE.ID_PROFE
            WHERE CURSO.NIVEL_CURSO=1 AND ASIGN.ID_PROFE=1 AND REL_CURSO_ASIGN.ID_ASIGN=1)
...
//query para para mostrar todos los grupos de un curso de un centro de un profe y que te muestre si están asignados actualmente o no 
SELECT CURSO.ID_CURSO AS groupId, CURSO.NIVEL_CURSO AS groupLevel, CURSO.LETRA_CURSO AS groupLetter, 0 as groupChecked FROM CURSO
	INNER JOIN CENTR ON CURSO.ID_CENTR = CENTR.ID_CENTR
	INNER JOIN PROFE ON CENTR.ID_CENTR = PROFE.ID_CENTR
	WHERE PROFE.ID_PROFE=1 AND CURSO.NIVEL_CURSO=1 AND CURSO.ID_CURSO NOT IN (SELECT REL_CURSO_ASIGN.ID_CURSO FROM REL_CURSO_ASIGN WHERE REL_CURSO_ASIGN.ID_ASIGN = 1)
UNION ALL
SELECT CURSO.ID_CURSO AS groupId, CURSO.NIVEL_CURSO AS groupLevel, CURSO.LETRA_CURSO AS groupLetter, 1 AS groupChecked FROM CURSO
			INNER JOIN REL_CURSO_ASIGN ON CURSO.ID_CURSO=REL_CURSO_ASIGN.ID_CURSO 
			INNER JOIN ASIGN ON REL_CURSO_ASIGN.ID_ASIGN=ASIGN.ID_ASIGN
...
SELECT CURSO.ID_CURSO AS groupId, CURSO.NIVEL_CURSO AS groupLevel, CURSO.LETRA_CURSO AS groupLetter, 1 AS groupChecked FROM CURSO
            ...
            INNER JOIN PROFE ON ASIGN.ID_PROFE=PROFE.ID_PROFE
            WHERE CURSO.NIVEL_CURSO=1 AND ASIGN.ID_PROFE=1 AND REL_CURSO_ASIGN.ID_ASIGN=1
//query para mostrar todos los grupos de un curso de un centro de un profe
SELECT CURSO.ID_CURSO AS groupId, CURSO.NIVEL_CURSO AS groupLevel, CURSO.LETRA_CURSO AS groupLetter FROM CURSO
INNER JOIN CENTR ON CURSO.ID_CENTR = CENTR.ID_CENTR
INNER JOIN PROFE ON CENTR.ID_CENTR = PROFE.ID_CENTR
WHERE PROFE.ID_PROFE=1 AND CURSO.NIVEL_CURSO=1

SELECT CURSO.ID_CURSO AS groupId, CURSO.NIVEL_CURSO AS groupLevel, CURSO.LETRA_CURSO AS groupLetter, 0 as noChecked FROM CURSO
INNER JOIN CENTR ON CURSO.ID_CENTR = CENTR.ID_CENTR
INNER JOIN PROFE ON CENTR.ID_CENTR = PROFE.ID_CENTR
WHERE PROFE.ID_PROFE=1 AND CURSO.NIVEL_CURSO=1 AND CURSO.ID_CURSO NOT IN (SELECT REL_CURSO_ASIGN.ID_CURSO FROM REL_CURSO_ASIGN WHERE REL_CURSO_ASIGN.ID_ASIGN = 1)
*/
