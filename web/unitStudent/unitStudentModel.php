	...
	class UnitStudentModel{
		...
		public function unitsStudent($IdAsign){
			...
			$consult = "SELECT ASIGN.NOMBRE_ASIGN AS NombreAsign, ASIGN.ID_ASIGN AS IdAsign, TEMAS.ID_TEMAS AS IdTema, TEMAS.ICON_TEMAS AS ImgTema,
						TEMAS.NOMBRE_TEMAS AS NombreTema, TEMAS.TRIMESTRE_TEMAS AS TrimTema, TEMAS.ORDEN_TEMAS AS OrdenTema
						FROM ASIGN
						INNER JOIN TEMAS ON ASIGN.ID_ASIGN = TEMAS.ID_ASIGN
						...
						WHERE ASIGN.ID_ASIGN = {$IdAsign} AND ALUMN.ID_ALUMN = '{$_SESSION["id"]}' ORDER BY TEMAS.ORDEN_TEMAS ASC";
			$result = $this->mysqli -> executeQuery($consult);
			return $result;
		}

		public function passedUnit($IdAsign){
			$_SESSION['id'] = 1;
			$consult = "SELECT FINAL.ID_FINAL AS IdFin, (IF (REL_ALUMN_FINAL.PUNTOS_REL_ALUMN_FINAL IS NOT NULL,
						...
						LEFT JOIN REL_ALUMN_FINAL ON FINAL.ID_FINAL = REL_ALUMN_FINAL.ID_FINAL
						LEFT JOIN ALUMN ON REL_ALUMN_FINAL.ID_ALUMN = ALUMN.ID_ALUMN
						WHERE ASIGN.ID_ASIGN = {$IdAsign} ORDER BY FINAL.ID_FINAL ASC";
			$result = $this->mysqli -> executeQuery($consult);
			return $result;
		}
