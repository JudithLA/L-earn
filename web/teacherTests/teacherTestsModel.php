...
require_once __DIR__ . "/../ConnectSupport/Implementation/MysqlDBImplementation.php";
	...
	class TeacherTestsModel{
		...
		public function teacherTestsQuery(){
			...
			$consult = "SELECT DISTINCT FINAL.ID_FINAL AS IdTestFinal, FINAL.NOMBRE_FINAL AS NombreTestFinal, FINAL.DESCR_FINAL AS DesTestFinal, ASIGN.ID_ASIGN AS AsignTest, CURSO.NIVEL_CURSO AS LevelTest FROM FINAL
					...
					WHERE ASIGN.ID_PROFE='{$_SESSION ["id"]}'
				UNION ALL
				SELECT DISTINCT ENTRE.ID_ENTRE AS IdTestEntre, ENTRE.NOMBRE_ENTRE AS NombreTestEntre, ENTRE.DESCR_ENTRE AS DesTestEntre,  ASIGN.ID_ASIGN AS AsignTest, CURSO.NIVEL_CURSO AS LevelTest FROM ENTRE
